            ...
            foreach (System.Reflection.PropertyInfo propertyInfo in p.GetType().GetProperties())
            {
                if (propertyInfo.CanRead)
                {
                    ...
                    if (propertyInfo.PropertyType == typeof(ObservableCollection<Attachment>))
                    {
                        ...
                        {
                            properties.Add(string.Format("{0}={1}", JsonProperty ?? propertyInfo.Name, HttpUtility.UrlEncode(JsonConvert.SerializeObject(propertyInfo.GetValue(p, null)))));
                        }
                    }
                    else
                    {
                        if (propertyInfo.GetValue(p, null) != null)
                        {
                            properties.Add(string.Format("{0}={1}", JsonProperty ?? propertyInfo.Name, HttpUtility.UrlEncode(propertyInfo.GetValue(p, null).ToString())));
                        }
                    }
